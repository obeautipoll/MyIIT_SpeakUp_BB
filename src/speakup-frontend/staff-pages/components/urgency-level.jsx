...
const UrgentComplaintsWidget = ({ staffEmail, staffRole }) => {
  ...
  const fetchAndAnalyzeComplaints = async () => {
    ...
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        // 🚨 Only High + Critical allowed for everyone
        if (urgency !== "High" && urgency !== "Critical") {
          continue;
        }

        // Normalize assignment values
        const assignedToDb = data.assignedTo?.toLowerCase().trim() || "";
        const assignedToUser = staffEmail?.toLowerCase().trim() || "";

        // 🧑‍💼 STAFF FILTER — staff MUST see ONLY their assigned complaints
        if (staffRole === "staff") {
          // If complaint is assigned AND not assigned to this staff → skip
          if (assignedToDb && assignedToDb !== assignedToUser) {
            continue;
          }
        }

        // ADMIN sees all urgent complaints; staff sees only theirs
